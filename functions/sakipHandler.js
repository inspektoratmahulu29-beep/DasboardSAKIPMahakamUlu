...
function jsonResponse(data, status = 200) {
  return new Response(JSON.stringify(data), {
    status,
    headers: { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*', 'Access-Control-Allow-Methods': 'GET, POST, OPTIONS', 'Access-Control-Allow-Headers': 'Content-Type', 'Cache-Control': 'no-store' },
  });
}
...
function formatNoteToRecommendation(noteItem) {
  ...
  return `Perbaiki kriteria ${id} (${kriteria}). ${normalizeText(cleanedNote).charAt(0).toUpperCase() + normalizeText(cleanedNote).slice(1)}`;
}
...
// OPTIMASI: Ambil semua data OPD dan Score sekaligus dalam 3 Query (Batch)
...
function getKriteriaStatus(data, source) { 
  ...
  data.forEach(row => { 
    const komp = row.Komponen; const grade = source === 'pm' ? row.pmGrade : row.inspGrade; const note = source === 'pm' ? row.pmNote : row.inspNote; 
    if (note && note.trim() !== "") hasil[komp].catatan.push({ id: row.ID, kriteria: normalizeText(row.Kriteria), note: cleanNote(note) }); 
    if (grade === "A" || grade === "B") hasil[komp].terpenuhi.push(row.ID + " - " + normalizeText(row.Kriteria)); 
    else hasil[komp].belum.push(row.ID + " - " + normalizeText(row.Kriteria)); 
  }); 
  return hasil; 
}

function buildRekomendasiRingkas(maxBobot, nilaiKomponen, data, source, catatanList) { 
  const rekomendasi = []; const komponenList = ["PERENCANAAN KINERJA", "PENGUKURAN KINERJA", "PELAPORAN KINERJA", "EVALUASI AKUNTABILITAS KINERJA INTERNAL"]; 
  komponenList.forEach(k => { if (maxBobot[k] > 0 && (nilaiKomponen[k] / maxBobot[k]) < 0.7) { 
    if (k === "PERENCANAAN KINERJA") { rekomendasi.push("Melakukan reviu dan penyempurnaan Pohon Kinerja serta cascading agar hubungan sebab-akibat antarindikator terlihat jelas."); rekomendasi.push("Menetapkan dan memperbaiki indikator kinerja utama berbasis outcome yang memenuhi prinsip SMART."); rekomendasi.push("Menyusun Manual Indikator/Profil Indikator untuk seluruh IKU."); } 
    else if (k === "PENGUKURAN KINERJA") { rekomendasi.push("Melaksanakan pengukuran serta rapat evaluasi kinerja secara berkala, sekurang-kurangnya setiap triwulan."); rekomendasi.push("Menyelaraskan Rencana Aksi dengan postur DPA/DPA Perubahan."); } 
    else if (k === "PELAPORAN KINERJA") { rekomendasi.push("Melengkapi LKjIP dengan reviu internal yang resmi dan berjenjang."); } 
    else if (k === "EVALUASI AKUNTABILITAS KINERJA INTERNAL") { rekomendasi.push("Membentuk secara resmi Tim Evaluator Mandiri melalui Surat Tugas."); rekomendasi.push("Menyusun tindak lanjut hasil evaluasi dalam bentuk laporan naratif."); } 
  } }); 
  if (catatanList) { catatanList.forEach(item => { const formatted = formatNoteToRecommendation(item); if (formatted) rekomendasi.push(formatted); }); } 
...
async function generateRekomendasiWithAI(env, kriteriaBelum, maxBobot, nilaiKomponen, data, source = 'pm') { 
  ...
  const catatan = []; if (data) data.forEach(row => { if (source === 'pm' && row.pmNote && row.pmNote.trim() !== "") catatan.push(`[Kriteria ${row.ID}] ${normalizeText(row.Kriteria)}: ${cleanNote(row.pmNote)}`); if (source === 'insp' && row.inspNote && row.inspNote.trim() !== "") catatan.push(`[Kriteria ${row.ID}] ${normalizeText(row.Kriteria)}: ${cleanNote(row.inspNote)}`); }); 
  ...
  const isValidList = (list) => { if (!list || list.length === 0) return false; return list.every(item => item && item.length > 5 && /[a-zA-Z]/.test(item) && !/^["'`]/.test(item)); }; 
  if (env.AI) { const models = ['@cf/meta/llama-3.3-70b-instruct-fp8-fast', '@cf/meta/llama-3.1-8b-instruct']; for (const model of models) { try { const response = await env.AI.run(model, { messages: [{ role: 'user', content: prompt }] }); if (response && response.response) { const list = response.response.split('\n').map(l => l.replace(/^[-*]\s*/, '').trim()).filter(l => l.length > 0); if (isValidList(list)) return { list, provider: `Workers AI (${model})` }; } } catch (e) { console.error(`Workers AI (${model}) gagal:`, e.message); } } } 
  if (env.AI_API_KEY) { try { const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash:generateContent?key=${env.AI_API_KEY}`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ contents: [{ parts: [{ text: prompt }] }] }) }); if (response.ok) { const data = await response.json(); const text = data.candidates[0].content.parts[0].text || ''; const list = text.split('\n').map(l => l.replace(/^[-*]\s*/, '').trim()).filter(l => l.length > 0); if (isValidList(list)) return { list, provider: "Google Gemini" }; } else { console.error('Gemini HTTP Error:', response.status, await response.text()); } } catch (e) { console.error('Gemini gagal:', e.message); } } 
  if (env.MISTRAL_API_KEY) { try { const response = await fetch('https://api.mistral.ai/v1/chat/completions', { method: 'POST', headers: { 'Authorization': `Bearer ${env.MISTRAL_API_KEY}`, 'Content-Type': 'application/json' }, body: JSON.stringify({ model: 'mistral-small-latest', messages: [{ role: 'user', content: prompt }] }) }); if (response.ok) { const data = await response.json(); const text = data.choices[0].message.content || ''; const list = text.split('\n').map(l => l.replace(/^[-*]\s*/, '').trim()).filter(l => l.length > 0); if (isValidList(list)) return { list, provider: "Mistral AI" }; } else { console.error('Mistral HTTP Error:', response.status, await response.text()); } } catch (e) { console.error('Mistral AI gagal:', e.message); } } 
  if (env.GROQ_API_KEY) { try { const response = await fetch('https://api.groq.com/openai/v1/chat/completions', { method: 'POST', headers: { 'Authorization': `Bearer ${env.GROQ_API_KEY}`, 'Content-Type': 'application/json' }, body: JSON.stringify({ model: 'llama3-8b-8192', messages: [{ role: 'user', content: prompt }] }) }); if (response.ok) { const data = await response.json(); const text = data.choices[0].message.content || ''; const list = text.split('\n').map(l => l.replace(/^[-*]\s*/, '').trim()).filter(l => l.length > 0); if (isValidList(list)) return { list, provider: "Groq" }; } else { console.error('Groq HTTP Error:', response.status, await response.text()); } } catch (e) { console.error('Groq gagal:', e.message); } } 
  console.warn('Semua AI Gagal. Menggunakan Fallback Template.'); const catatanList = []; Object.keys(kriteriaBelum).forEach(komp => { if (kriteriaBelum[komp] && kriteriaBelum[komp].catatan) catatanList.push(...kriteriaBelum[komp].catatan); }); return { list: buildRekomendasiRingkas(maxBobot, nilaiKomponen, data, source, catatanList), provider: "Template" }; }

async function generateClosingWithAI(env, data, totalNilai, predikat, opdName, year, source) { 
  const komponenList = ["PERENCANAAN KINERJA", "PENGUKURAN KINERJA", "PELAPORAN KINERJA", "EVALUASI AKUNTABILITAS KINERJA INTERNAL"]; const maxBobot = { "PERENCANAAN KINERJA": 0, "PENGUKURAN KINERJA": 0, "PELAPORAN KINERJA": 0, "EVALUASI AKUNTABILITAS KINERJA INTERNAL": 0 }; const nilaiKomponen = { "PERENCANAAN KINERJA": 0, "PENGUKURAN KINERJA": 0, "PELAPORAN KINERJA": 0, "EVALUASI AKUNTABILITAS KINERJA INTERNAL": 0 }; const gradeField = source === 'pm' ? 'pmGrade' : 'inspGrade'; 
  ...
  let totalMax = komponenList.reduce((sum, k) => sum + maxBobot[k], 0); let weakestComp = komponenList[0], highestComp = komponenList[0]; let minPct = 999, maxPct = -1; 
  komponenList.forEach(k => { const pct = totalMax > 0 ? (nilaiKomponen[k] / totalMax * 100) : 0; if (pct < minPct) { minPct = pct; weakestComp = k; } if (pct > maxPct) { maxPct = pct; highestComp = k; } }); 
  ...
  if (env.AI) { const models = ['@cf/meta/llama-3.3-70b-instruct-fp8-fast', '@cf/meta/llama-3.1-8b-instruct']; for (const model of models) { try { const response = await env.AI.run(model, { messages: [{ role: 'user', content: prompt }] }); if (response && response.response) return { text: response.response.trim(), provider: `Workers AI (${model})` }; } catch (e) { console.error(`Closing Workers AI (${model}) gagal:`, e.message); } } } 
...
async function generateLaporanHtml({ year, opdName, env, source }) { 
  ...
  const formattedOpdName = formatOPDName(opdName); 
  const groupedData = {}; 
  data.forEach(row => { if (!row.RuleMap) row.RuleMap = {}; if (!groupedData[row.Komponen]) groupedData[row.Komponen] = { totalBobot: 0, totalNilai: 0, subKomponen: {} }; if (!groupedData[row.Komponen].subKomponen[row.SubKomponen]) groupedData[row.Komponen].subKomponen[row.SubKomponen] = { totalBobot: 0, totalNilai: 0, kriteria: [] }; const nilai = row.RuleMap[source === 'pm' ? row.pmGrade : row.inspGrade] || 0; groupedData[row.Komponen].totalBobot += row.Bobot; groupedData[row.Komponen].totalNilai += nilai; groupedData[row.Komponen].subKomponen[row.SubKomponen].totalBobot += row.Bobot; groupedData[row.Komponen].subKomponen[row.SubKomponen].totalNilai += nilai; groupedData[row.Komponen].subKomponen[row.SubKomponen].kriteria.push(row); }); 
  let totalNilai = 0, totalMax = 0; komponenList.forEach(k => { if (groupedData[k]) { totalNilai += groupedData[k].totalNilai; totalMax += groupedData[k].totalBobot; } }); const predikat = getPredikat(totalNilai); const prevScores = await getPrevScores(year, opdName, env); const statusKriteria = getKriteriaStatus(data, source); const aiResult = await generateRekomendasiWithAI(env, statusKriteria, maxBobot, nilaiKomponen, data, source); const aiClosing = await generateClosingWithAI(env, data, totalNilai, predikat, formattedOpdName, year, source); const rekomendasi = aiResult.list; const aiProvider = aiResult.provider; const closingParagraph = aiClosing.text; 
  let html = `<html><head><title>LHE ${source === 'pm' ? 'PM' : 'INSP'} SAKIP ${formattedOpdName} TA ${year}</title></head>`; html += `<body style="font-family: 'Times New Roman', Times, serif; font-size: 12pt; line-height: 1.5; margin: 2cm; text-align: justify;">`; html += `<div style="text-align:center; margin-bottom: 20px;"><h2 style="margin:0; font-size:14pt; font-weight:bold;">PEMERINTAH KABUPATEN MAHAKAM ULU</h2><h2 style="margin:0; font-size:14pt; font-weight:bold;">${formattedOpdName}</h2><p style="margin:0; font-size:10pt;">Jalan Gunung Belareq Gg. Dunhil RT. VII Kampung Ujoh Bilang Kecamatan Long Bagun</p><p style="margin:0; font-size:10pt;">UJOH BILANG</p><hr style="border:1px solid black; margin:10px 0;"></div>`; html += `<div style="text-align:center; margin-bottom: 20px;"><h2 style="margin:0; font-size:14pt; font-weight:bold;">LAPORAN HASIL EVALUASI ${source === 'pm' ? 'PENILAIAN MANDIRI (LHE PM)' : 'PENILAIAN INSPEKTORAT (LHE INSP)'}</h2><h2 style="margin:0; font-size:14pt; font-weight:bold;">AKUNTABILITAS KINERJA INSTANSI PEMERINTAH (AKIP)</h2><h2 style="margin:0; font-size:14pt; font-weight:bold;">${formattedOpdName} KABUPATEN MAHAKAM ULU ${year}</h2><p style="margin:0; font-size:10pt;">Nomor: ....../..../LHE-${source === 'pm' ? 'PM' : 'INSP'}/${opdName}/2026</p></div>`; html += `<h3 style="font-size:12pt; font-weight:bold; margin-top:20px;">I. PENDAHULUAN</h3><p>Laporan Hasil Evaluasi ${source === 'pm' ? 'Penilaian Mandiri (LHE PM)' : 'Penilaian Inspektorat (LHE INSP)'} Akuntabilitas Kinerja Instansi Pemerintah (AKIP) ${formattedOpdName} Kabupaten Mahakam Ulu Tahun Anggaran ${year} disusun sebagai potret kondisi akuntabilitas kinerja perangkat daerah berdasarkan hasil telaah atas dokumen dan catatan evaluasi SAKIP yang tersedia.</p>`; html += `<p>Evaluasi difokuskan pada ketersediaan bukti dukung, kualitas implementasi, serta pemanfaatan SAKIP pada empat komponen utama sesuai kerangka evaluasi dalam Peraturan Menteri Pendayagunaan Aparatur Negara dan Reformasi Birokrasi Nomor 88 Tahun 2021, Peraturan Bupati Mahakam Ulu Nomor 1 Tahun 2026 tentang Evaluasi Akuntabilitas Kinerja Instansi Pemerintah.</p>`; html += `<h4 style="font-size:12pt; font-weight:bold; margin-top:15px;">A. Dasar Hukum Evaluasi</h4><p>Sebagai landasan pijak yang memperkuat langkah kita bersama dalam mewujudkan tata kelola pemerintahan yang baik, pelaksanaan evaluasi atas Sistem Akuntabilitas Kinerja Instansi Pemerintah (SAKIP) di lingkungan ${formattedOpdName} Kabupaten Mahakam Ulu berpedoman pada regulasi berikut:</p><ol>`; html += `<li>Undang-Undang Nomor 23 Tahun 2014 tentang Pemerintahan Daerah sebagaimana telah beberapa kali diubah terakhir dengan Undang-Undang Nomor 9 Tahun 2015.</li><li>Peraturan Presiden Republik Indonesia Nomor 29 Tahun 2014 tentang Sistem Akuntabilitas Kinerja Instansi Pemerintah.</li><li>Peraturan Pemerintah Nomor 12 Tahun 2017 tentang Pembinaan dan Pengawasan Penyelenggaraan Pemerintah Daerah.</li><li>Peraturan Pemerintah Nomor 13 Tahun 2019 tentang Pelaporan dan Evaluasi Penyelenggaraan Pemerintah Daerah.</li><li>Peraturan Menteri Pendayagunaan Aparatur Negara dan Reformasi Birokrasi Nomor 88 Tahun 2021 tentang Pedoman Evaluasi Akuntabilitas Kinerja Instansi Pemerintah.</li><li>Peraturan Daerah Kabupaten Mahakam Ulu Nomor 14 Tahun 2016 tentang Pembentukan dan Susunan Perangkat Daerah, serta Peraturan Bupati Mahakam Ulu Nomor 27 Tahun 2016 tentang Susunan Organisasi dan Tata Kerja Perangkat Daerah.</li><li>Peraturan Bupati Mahakam Ulu Nomor 1 Tahun 2026 tentang Evaluasi Akuntabilitas Kinerja Instansi Pemerintah.</li><li>Keputusan Bupati Mahakam Ulu Nomor [700.1.1/K.6a/2025] tentang Program Kerja Pengawasan Tahunan (PKPT) Berbasis Risiko, yang ditindaklanjuti dengan Surat Perintah Tugas Inspektur Inspektorat Nomor: [090/20/INSPEKTORAT/III/2026 tanggal 02 Maret 2026.]</li></ol>`; html += `<h4 style="font-size:12pt; font-weight:bold; margin-top:15px;">B. Latar Belakang Evaluasi</h4><p>Saat ini terus bergerak maju dalam menyempurnakan tata kelola birokrasinya. Kita bersama-sama sedang berada dalam masa transisi yang positif, bergeser dari budaya kerja yang sekadar berfokus pada kelengkapan administrasi dan penyerapan anggaran, menuju budaya kerja yang benar-benar memberikan hasil (outcome) dan manfaat nyata bagi masyarakat luas. Dalam perjalanan mulia ini, SAKIP hadir bukan sebagai beban tambahan, melainkan sebagai instrumen navigasi yang membantu kita memastikan bahwa setiap program dan anggaran berjalan di jalur yang tepat.</p>`; html += `<h3 style="font-size:12pt; font-weight:bold; margin-top:20px;">II. GAMBARAN UMUM HASIL EVALUASI</h3><p>Secara keseluruhan, ${formattedOpdName} memperoleh nilai ${source === 'pm' ? 'Penilaian Mandiri' : 'Penilaian Inspektorat'}/hasil evaluasi sebesar ${totalNilai.toFixed(2)} dengan predikat ${predikat}. Nilai tersebut merupakan hasil akumulasi empat komponen SAKIP.</p>`; html += `<table border="1" style="border-collapse: collapse; width: 100%; table-layout: fixed; margin-top: 10px; font-size: 10pt;">`; html += `<colgroup><col style="width: 5%;"><col style="width: 45%;"><col style="width: 20%;"><col style="width: 30%;"></colgroup>`; if (source === 'pm') { html += `<tr style="background: #e8e8e8;"><th style="padding: 6px; width: 5%;">No</th><th style="padding: 6px; width: 45%;">Komponen / Sub Komponen / Kriteria</th><th style="padding: 6px; width: 20%;">Bobot</th><th style="padding: 6px; width: 30%;">Nilai PM</th></tr>`; } else { html += `<tr style="background: #e8e8e8;"><th style="padding: 6px; width: 5%;">No</th><th style="padding: 6px; width: 45%;">Komponen / Sub Komponen / Kriteria</th><th style="padding: 6px; width: 20%;">Bobot</th><th style="padding: 6px; width: 30%;">Nilai Inspektorat</th></tr>`; } komponenList.forEach((komponen, idxKomponen) => { const kompGroup = groupedData[komponen]; if (!kompGroup) return; html += `<tr style="background: #d1e7dd; font-weight: bold;"><td style="padding: 6px; text-align:center;">${idxKomponen + 1}</td><td style="padding: 6px;">${komponen} (${kompGroup.totalBobot}%)</td><td style="padding: 6px; text-align:center;">${kompGroup.totalBobot.toFixed(2)}</td><td style="padding: 6px; text-align:center;">${kompGroup.totalNilai.toFixed(2)}</td></tr>`; Object.keys(kompGroup.subKomponen).forEach(subKey => { const subGroup = kompGroup.subKomponen[subKey]; html += `<tr style="background: #f8f9fa; font-weight: bold;"><td style="padding: 6px;"></td><td style="padding: 6px; padding-left: 20px;">${subKey} (${subGroup.totalBobot}%)</td><td style="padding: 6px; text-align:center;">${subGroup.totalBobot.toFixed(2)}</td><td style="padding: 6px; text-align:center;">${subGroup.totalNilai.toFixed(2)}</td></tr>`; subGroup.kriteria.forEach((row, idxKriteria) => { const pmScore = row.RuleMap[row.pmGrade] || 0; const inspScore = row.RuleMap[row.inspGrade] || 0; html += `<tr><td style="padding: 6px; text-align:center; word-wrap: break-word;">${idxKriteria + 1}</td><td style="padding: 6px; padding-left: 40px; word-wrap: break-word;">${normalizeText(row.Kriteria)}</td><td style="padding: 6px; text-align:center;">${row.Bobot}</td><td style="padding: 6px; text-align:center;">${source === 'pm' ? pmScore.toFixed(2) : inspScore.toFixed(2)}</td></tr>`; }); }); }); html += `</table>`; html += `<h3 style="font-size:12pt; font-weight:bold; margin-top:20px;">III. ANALISIS PER KOMPONEN</h3>`; komponenList.forEach((k, idx) => { const nilai = groupedData[k] ? groupedData[k].totalNilai : 0; const bobot = groupedData[k] ? groupedData[k].totalBobot : 0; const pct = totalMax > 0 ? (nilai / totalMax * 100).toFixed(2) : "0.00"; html += `<h4 style="font-size:12pt; font-weight:bold; margin-top:10px;">${idx+1}. ${k} - nilai ${nilai.toFixed(2)} dari maksimal ${bobot.toFixed(2)}</h4><p>Komponen ini memperoleh nilai ${nilai.toFixed(2)} dari maksimal ${bobot.toFixed(2)}. Kontribusi terhadap total keseluruhan: ${pct}%.</p>`; const status = statusKriteria[k] || { terpenuhi: [], belum: [], catatan: [] }; if (status.terpenuhi.length > 0) { html += `<p><b>Kriteria yang sudah terpenuhi:</b></p><ul>`; status.terpenuhi.forEach(item => html += `<li>${item}</li>`); html += `</ul>`; } if (status.belum.length > 0) { html += `<p><b>Kriteria yang belum terpenuhi:</b></p><ul>`; status.belum.forEach(item => html += `<li>${item}</li>`); html += `</ul>`; } if (status.catatan.length > 0) { html += `<p><b>Catatan ${source === 'pm' ? 'Penilaian Mandiri' : 'Inspektorat'}:</b></p><ul>`; status.catatan.forEach(cat => { html += `<li>${formatNoteToRecommendation(cat)}</li>`; }); html += `</ul>`; } }); html += `<h3 style="font-size:12pt; font-weight:bold; margin-top:20px;">IV. REKOMENDASI PERBAIKAN</h3><ul>`; rekomendasi.forEach(r => html += `<li>${normalizeText(r)}</li>`); html += `</ul>`; html += `<h3 style="font-size:12pt; font-weight:bold; margin-top:20px;">V. PENUTUP</h3><p>${closingParagraph}</p>`; html += `<br><br><div style="text-align:right;"><p style="margin:0;">Ujoh Bilang, ${new Date().toLocaleDateString('id-ID', { month: 'long', year: 'numeric' })}</p><p style="margin:0;">${source === 'pm' ? `Kepala ${formattedOpdName}` : 'Inspektur Kabupaten Mahakam Ulu'}</p><br><br><p style="margin:0;">_______________________</p><p style="margin:0;">Nama Lengkap</p><p style="margin:0;">NIP. ............................</p></div>`; html += `</body></html>`; return { html, aiProvider, totalNilai, predikat }; }
...
export const onRequest = async ({ request, env }) => { 
  ...
  try { 
    switch (action) { 
      ...
      case 'getDashboardData': { const cacheUrl = new URL(request.url); const cacheKey = new Request(cacheUrl.toString()); const cache = caches.default; const cached = await cache.match(cacheKey); if (cached) return cached; const bulk = await getBulkData(year, env); let totalOPD = bulk.length; let minPM = Infinity, minInsp = Infinity; let topPM = {name:'', value:-1}, topInsp = {name:'', value:-1}; let qaCount = {selesai:0,proses:0,belum:0}; let evidenceLengkapCount = 0; let totalProgress = 0; if (totalOPD > 0) { bulk.forEach(o => { const pm = parseFloat(o.pmTotal); const insp = parseFloat(o.inspTotal); const progress = parseFloat(o.progress); if (pm > 0 && pm < minPM) minPM = pm; if (insp > 0 && insp < minInsp) minInsp = insp; if (pm > topPM.value) topPM = {name: o.opd_name, value: pm}; if (insp > topInsp.value) topInsp = {name: o.opd_name, value: insp}; if (o.qaApipStatus === 'Selesai') qaCount.selesai++; else if (o.qaApipStatus === 'Proses') qaCount.proses++; else qaCount.belum++; totalProgress += progress; if (pm > 0) evidenceLengkapCount++; }); } if (minPM === Infinity) minPM = 0; if (minInsp === Infinity) minInsp = 0; const respBody = { year, totalOPD, topPM: { name: topPM.name || 'Belum ada', value: (Number(topPM.value)||0).toFixed(2) }, topInsp: { name: topInsp.name || 'Belum ada', value: (Number(topInsp.value)||0).toFixed(2) }, minPM: (Number(minPM)||0).toFixed(2), minInsp: (Number(minInsp)||0).toFixed(2), avgProgress: (Number(totalProgress)/totalOPD).toFixed(0) + '%', qaCount, evidenceLengkap: (evidenceLengkapCount/totalOPD*100).toFixed(0) + '%' }; const res = new Response(JSON.stringify(respBody), { status: 200, headers: { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*', 'Cache-Control': 'public, max-age=30' } }); await cache.put(cacheKey, res.clone()); return res; }
      ...
      case 'uploadEvidence': { const { base64Data, opdName, criteriaId, fileName, mimeType } = params; if (base64Data.length > 20 * 1024 * 1024) return jsonResponse({ status: 'error', msg: 'File melebihi batas 15MB!' }); const bytes = Uint8Array.from(atob(base64Data), c => c.charCodeAt(0)); const r2Path = `sakip/${year}/${opdName}/${criteriaId}/${Date.now()}_${fileName}`; await env.EVIDENCE_BUCKET.put(r2Path, bytes, { httpMetadata: { contentType: mimeType || 'application/octet-stream' } }); const publicUrl = `https://pub-6825f3819d9d46089a296f5d492fab22.r2.dev/${r2Path}`; let gdriveId = null; if (env.GOOGLE_DRIVE_CLIENT_ID && env.GOOGLE_DRIVE_CLIENT_SECRET && env.GOOGLE_DRIVE_REFRESH_TOKEN && env.GOOGLE_DRIVE_FOLDER_ID) { try { gdriveId = await uploadToGoogleDrive(env, r2Path, fileName, bytes, env.GOOGLE_DRIVE_FOLDER_ID); } catch (err) { console.error('Gagal upload ke Google Drive:', err.message); } } await env.DB.prepare("INSERT INTO evidence (year, opd_name, criteria_id, url, gdrive_id, file_name) VALUES (?, ?, ?, ?, ?, ?)").bind(year, opdName, criteriaId, publicUrl, gdriveId, fileName).run(); return jsonResponse({ status: 'success', url: publicUrl, gdriveId }); }
