...
async function getPMDataForInspectorData(year, opdName, env) {
  ...
  const scoreMap = {};
  dataScores.results.forEach(row => { scoreMap[row.criteria_id] = row; });
...
function getKriteriaStatus(data, source) {
  ...
  data.forEach(row => {
    ...
    if (grade === "A" || grade === "B") {
      hasil[komp].terpenuhi.push(row.ID + " - " + row.Kriteria);
    } else {
      hasil[komp].belum.push(row.ID + " - " + row.Kriteria);
    }
  });
  return hasil;
}
...
async function generateLaporanHtml({ year, opdName, env, source }) {
  ...
  html += `<p>Secara keseluruhan, ${opdName} memperoleh nilai ${source === 'pm' ? 'Penilaian Mandiri' : 'Penilaian Inspektorat'}/hasil evaluasi sebesar ${totalNilai.toFixed(2)} dengan predikat ${predikat}. Nilai tersebut merupakan hasil akumulasi empat komponen SAKIP.</p>`;
  ...
  komponenList.forEach((k, idx) => {
    ...
    if (status.terpenuhi.length > 0) {
      ...
      status.terpenuhi.forEach(item => html += `<li>${item}</li>`);
      html += `</ul>`;
    }
    if (status.belum.length > 0) {
      html += `<p><b>Kriteria yang belum terpenuhi:</b></p><ul>`;
      status.belum.forEach(item => html += `<li>${item}</li>`);
      html += `</ul>`;
    }
  ...
  try {
    switch (action) {
      ...
      case 'getYears': {
        ...
        return jsonResponse([...new Set(years)].sort((a,b) => b - a));
      }
      case 'addYear': {
        await env.DB.prepare("INSERT OR IGNORE INTO years (year) VALUES (?)").bind(params.year).run();
        return jsonResponse({ status: 'success', msg: 'Tahun ' + params.year + ' berhasil ditambahkan.' });
      }
      case 'deleteYear': {
      ...
      case 'saveInspData': {
        ...
        for (let i = 0; i < master.length; i++) {
          ...
          if (existing) {
            if (role === 'pm') await env.DB.prepare("UPDATE data_scores SET pm_grade = ?, pm_note = ? WHERE id = ?").bind(item.grade || '', item.note || '', existing.id).run();
            else await env.DB.prepare("UPDATE data_scores SET insp_grade = ?, insp_note = ? WHERE id = ?").bind(item.grade || '', item.note || '', existing.id).run();
          } else {
            if (role === 'pm') await env.DB.prepare("INSERT INTO data_scores (year, opd_name, criteria_id, pm_grade, pm_note) VALUES (?, ?, ?, ?, ?)").bind(year, opdName, critId, item.grade || '', item.note || '').run();
            else await env.DB.prepare("INSERT INTO data_scores (year, opd_name, criteria_id, insp_grade, insp_note) VALUES (?, ?, ?, ?, ?)").bind(year, opdName, critId, item.grade || '', item.note || '').run();
          }
        }
      ...
      case 'getDashboardData': {
        ...
        for (const opd of opds.results) {
          ...
          if (qaStatus === 'Selesai') qaCount.selesai++; else if (qaStatus === 'Proses') qaCount.proses++; else qaCount.belum++;
      ...
      case 'getChartData': {
        ...
        for (const opd of opds.results) {
          ...
          data.forEach(row => { pmTotal += row.RuleMap[row.pmGrade] || 0; inspTotal += row.RuleMap[row.inspGrade] || 0; if (row.qaApipStatus) status = row.qaApipStatus; });
          labels.push(opd.opd_name); pmScores.push(Number(pmTotal).toFixed(2)); inspScores.push(Number(inspTotal).toFixed(2)); qaStatus.push(status);
        }
      ...
      case 'deleteEvidence': {
        ...
        if (gdriveId) {
          try { await deleteGoogleDriveFile(env, gdriveId); } catch (err) { return jsonResponse({ status: 'error', msg: 'Gagal hapus di Google Drive: ' + err.message }); }
        }
      ...
      // ====== GENERATE LAPORAN PM / OPD ======
      case 'generateLaporanMandiri': {
        const { opdName } = params;
        const { html, aiProvider } = await generateLaporanHtml({ year, opdName, env, source: 'pm' });

        const bytes = new TextEncoder().encode(html);
        const r2Path = `laporan/${year}/PM_${opdName}_${Date.now()}.html`;
        ...
        let gdocsUrl = null;
        if (env.GOOGLE_DRIVE_CLIENT_ID && env.GOOGLE_DRIVE_CLIENT_SECRET && env.GOOGLE_DRIVE_REFRESH_TOKEN && env.GOOGLE_DRIVE_FOLDER_ID) {
          try { gdocsUrl = await createGoogleDoc(env, html, `LHE_PM_${opdName}_${year}`, env.GOOGLE_DRIVE_FOLDER_ID); }
          catch (e) { console.error('Gagal membuat Google Docs:', e); }
        }

        return jsonResponse({ status: 'success', url: laporanUrl, gdocsUrl: gdocsUrl, aiProvider, type: 'PM' });
      }

      // ====== GENERATE LAPORAN INSPEKTORAT / APIP ======
      case 'generateLaporanInspektorat': {
        const { opdName } = params;
        const { html, aiProvider } = await generateLaporanHtml({ year, opdName, env, source: 'insp' });

        const bytes = new TextEncoder().encode(html);
        const r2Path = `laporan/${year}/INSP_${opdName}_${Date.now()}.html`;
        await env.EVIDENCE_BUCKET.put(r2Path, bytes, { httpMetadata: { contentType: 'text/html' } });
        const laporanUrl = `https://pub-6825f3819d9d46089a296f5d492fab22.r2.dev/${r2Path}`;

        let gdocsUrl = null;
        if (env.GOOGLE_DRIVE_CLIENT_ID && env.GOOGLE_DRIVE_CLIENT_SECRET && env.GOOGLE_DRIVE_REFRESH_TOKEN && env.GOOGLE_DRIVE_FOLDER_ID) {
          try { gdocsUrl = await createGoogleDoc(env, html, `LHE_INSP_${opdName}_${year}`, env.GOOGLE_DRIVE_FOLDER_ID); }
          catch (e) { console.error('Gagal membuat Google Docs:', e); }
        }

        return jsonResponse({ status: 'success', url: laporanUrl, gdocsUrl: gdocsUrl, aiProvider, type: 'INSP' });
      }

      default:
        return jsonResponse({ status: 'error', msg: 'Aksi tidak dikenal: ' + action });
    }
  } catch (err) {
